...
void ImageViewer::render(const bool p_focus)
{
   ...
   if (m_image != NULL)
   {
      if (m_fitToScreen && (m_imageW > g_screen_width || m_imageH > g_screen_height - LINE_HEIGHT))
      {
         // Resize image to fit the screen
         double aspectRatioScreen = static_cast<double>(g_screen_width) / (g_screen_height - LINE_HEIGHT);
         double aspectRatioImage = static_cast<double>(m_imageW) / m_imageH;
         double resizeFactor = 0.0;
         if (aspectRatioImage > aspectRatioScreen)
            resizeFactor = static_cast<double>(m_imageW) / g_screen_width;
         else
            resizeFactor = static_cast<double>(m_imageH) / (g_screen_height - LINE_HEIGHT);
         int imageFitW = static_cast<double>(m_imageW) / resizeFactor;
         int imageFitH = static_cast<double>(m_imageH) / resizeFactor;
         SDL_Rect destRect = { (g_screen_width - imageFitW) / 2, LINE_HEIGHT + ((g_screen_height - LINE_HEIGHT - imageFitH) / 2), imageFitW, imageFitH };
         SDL_RenderCopy(g_renderer, m_image, NULL, &destRect);
      }
      else
      {
         // Display image as it is
         SDLUtils::renderTexture(m_image, (m_imageW > g_screen_width ? m_camera.x : 0) + (g_screen_width / 2), (m_imageH > g_screen_height - LINE_HEIGHT ? m_camera.y : 0) + ((g_screen_height + LINE_HEIGHT) / 2), SDLUtils::T_ALIGN_CENTER, SDLUtils::T_ALIGN_MIDDLE);
      }
   }
   else
   {
      // Error
      SDLUtils::renderText("Unable to load image", g_font, g_screen_width / 2, (g_screen_height + LINE_HEIGHT) / 2, {COLOR_TEXT_NORMAL}, {COLOR_BODY_BG}, SDLUtils::T_ALIGN_CENTER, SDLUtils::T_ALIGN_MIDDLE);
   }
